...
export function TeeemDocumentView<T extends DocumentItem>({
  ...
  const StatusBadge = ({ status }: { status: "draft" | "approved" }) => {
    ...
    return (
      <Badge
        variant="secondary"
        className="bg-muted text-foreground dark:bg-card dark:text-muted-foreground whitespace-nowrap"
      >
        {statusLabels.draft}
      </Badge>
    );
    ...
    return (
      <div className="h-full relative">
        {hasThumbnail ? (
          ...
          <div
            className="h-full w-full flex items-center justify-center bg-muted dark:bg-background cursor-pointer group"
            onClick={() => {
              setShowFullPdf(true);
