 ...
 *
 * Allows selecting which tasks should be automatically completed when this task is marked as complete.
 * Useful for milestone tasks or cleanup tasks that signal the completion of multiple other tasks.
 */

export default function AutoCompleteTasksModal({ isOpen, onClose, currentRow, allRows, onSave }) {
  const [selectedTaskIds, setSelectedTaskIds] = useState([])

  useEffect(() => {
    if (isOpen && currentRow) {
      // Initialize from current row's auto_complete_task_ids
      setSelectedTaskIds(currentRow.auto_complete_task_ids || [])
    }
  }, [isOpen, currentRow])
  ...
  const availableTasks = allRows
    ...
    .filter(row => row.id !== currentRow.id) // Can't auto-complete itself
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              Select tasks that should be automatically completed when Task #{currentTaskNumber}: {currentRow?.name} is completed
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="mx-6 mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
          ...
          <p className="text-xs text-blue-800 dark:text-blue-400">
            When this task is marked as complete, the selected tasks below will automatically be marked as complete
            (if they aren't already). This is useful for milestone tasks or final inspection tasks that signal the
            completion of multiple prerequisite tasks.
          </p>
        </div>
