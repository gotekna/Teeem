...
export interface EmailComposePaneProps {
  /** Whether the pane is currently active/visible. Controls data fetching and auto-save. */
  isActive: boolean;
  /** Called when the user wants to close/dismiss the compose pane */
  onClose: () => void;
  defaultTo?: string;
  defaultCc?: string;
...
export function EmailComposePane({
  ...
  // Generate signature from current user data
  ...
  useEffect(() => {
    ...
    if (isActive && !companySettings) {
      fetchCompanySettings();
    }
  }, [isActive, companySettings]);
  ...
  // Fetch accounts when pane becomes active
  ...
  }, [isActive, hasInitialized, defaultTo, defaultCc, defaultSubject, defaultBody, draft, initialAttachments, initialExistingStorageKeys, initialPreUploadedAttachments]);

  // Fetch forward attachments
  useEffect(() => {
    if (!isActive || !forwardEmailId || !forwardAttachments?.length) return;
  ...
  }, [isActive, forwardEmailId, forwardAttachments]);

  // Include original email attachments on demand
  ...
  // Generate signature when account is selected
  useEffect(() => {
    if (!isActive) return;
  ...
  }, [isActive, formData.credential_id, accounts, currentUser, companySettings]);
