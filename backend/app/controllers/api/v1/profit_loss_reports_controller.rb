...
module Api
  module V1
    class ProfitLossReportsController < ApplicationController
      before_action :set_company

      # GET /api/v1/companies/:company_id/profit_loss_reports
      def index
        reports = @company.profit_loss_reports.order(financial_year: :desc)
      ...
      # GET /api/v1/companies/:company_id/profit_loss_reports/:id
      def show
      ...
      # POST /api/v1/companies/:company_id/profit_loss_reports/generate
      def generate
        financial_year = params[:financial_year] || current_financial_year

        # Find or create report for this FY
        report = @company.profit_loss_reports.find_or_initialize_by(financial_year: financial_year)

        if report.new_record?
          report.company_name = @company.name
          report.company_code = @company.short_code.presence || @company.name[0..1].upcase
          report.period_start = financial_year_start(financial_year)
          report.period_end = financial_year_end(financial_year)
          report.save!
        end
      ...
      # POST /api/v1/companies/:company_id/profit_loss_reports/:id/regenerate
      def regenerate
        report = @company.profit_loss_reports.find(params[:id])
        report.generate!
      ...
      def serialize_report(report, include_url: false)
        data = {
          ...
          total_revenue: report.total_revenue&.to_f,
          total_expenses: report.total_expenses&.to_f,
          net_profit: report.net_profit&.to_f,
