  ...
  module V1
    class DocumentsController < ApplicationController
      ...
      def warehouse
        ...
        # SM Task info: find tasks on this job that require document types belonging to this folder
        sm_task_info = nil
        if params[:warehouse_folder_id].present? && params[:linkable_type] == "Job" && params[:linkable_id].present?
          sm_task_info = build_sm_task_info_for_documents(
            params[:warehouse_folder_id].to_i,
            params[:linkable_id].to_i
          )
        end
