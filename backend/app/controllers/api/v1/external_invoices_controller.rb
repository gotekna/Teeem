      ...
      def by_job
        ...
        render json: {
          ...
          data: {
            ...
            invoices_total: sales_invoices.sum(:total)&.to_f || 0,
            invoices_total_ex_gst: sales_invoices.sum(:subtotal)&.to_f || 0,
            invoices_paid: sales_invoices.sum(:amount_paid)&.to_f || 0,
            invoices_due: sales_invoices.sum(:amount_due)&.to_f || 0,
            bills_total: bills.sum(:total)&.to_f || 0,
            bills_total_ex_gst: bills.sum(:subtotal)&.to_f || 0,
            bills_paid: bills.sum(:amount_paid)&.to_f || 0,
            bills_due: bills.sum(:amount_due)&.to_f || 0,
            job_id: job.id,
            job_title: job.title,
