      ...
      class AuthenticationController < ApplicationController
        ...
        def login
          ...
          if portal_user.locked?
            render_error("Account is locked. Please try again later.", status: :forbidden)
            return
          end

          # Check if account is active
          unless portal_user.active?
            render_error("Account is not active. Please contact your builder.", status: :forbidden)
            return
          end
          ...
          render_error("An error occurred during login", status: :internal_server_error)
        end
        ...
        def reset_password
          ...
          unless token.present? && new_password.present?
            render_error("Token and password are required", status: :bad_request)
            return
          end
