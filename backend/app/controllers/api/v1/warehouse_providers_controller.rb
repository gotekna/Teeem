    ...
    class WarehouseProvidersController < ApplicationController
      ...
      def update
        ...
        if sp.key?(:warehouse_folders)
          sp[:warehouse_folders].to_h.each do |warehouse_type, template|
            folder = WarehouseFolder.warehouse_folder_for(warehouse_type)
            if folder
              ...
              if template.present?
                folder.update!(folder_path_suffix: template)
                Rails.logger.info "[WarehouseProvider] Saved folder_path_suffix for #{warehouse_type}: #{template}"
              else
        ...
        # Update link_expiry_days in TenantSetting (SSoT)
        if sp.key?(:link_expiry_days)
          TenantSetting.instance.update!(link_expiry_days: sp[:link_expiry_days].to_i)
        end
      ...
      def storage_params
        ...
        params.require(:storage).permit(
          ...
          # - warehouse_folders columns (paths now per-tab in warehouse_folders table)
