module Api
  module V1
    class SystemController < ApplicationController
      ...
      # Reads pre-computed payload from cache (written by QueueStatusCacheJob every 30s).
      # This keeps the web dyno at ~0 queries and <50ms response time.
      # Fallback: if cache is cold (first boot), compute inline once.
      def queue_status
        cached = Rails.cache.read(QueueStatusCacheJob::CACHE_KEY)

        if cached
          render json: { success: true, data: cached }
        else
          # Cache cold (first boot or job hasn't run yet) - compute inline once.
          # Call the job's computation directly (bypasses DeduplicatableJob callbacks).
          Rails.logger.info("[SystemController] queue_status cache miss - computing inline")
          job = QueueStatusCacheJob.new
          job.perform # Computes payload and writes to cache
          cached = Rails.cache.read(QueueStatusCacheJob::CACHE_KEY)
          render json: { success: true, data: cached || { status: "unknown", statusMessage: "Initializing..." } }
        end
      ...
      # Methods below moved to QueueStatusCacheJob (Feb 2026):
      # compute_queue_status, auto_clear_stale_failures, compute_backlog,
      # compute_db_connections, compute_memory_usage, compute_uptime,
      # humanize_duration, compute_xero_rate_limits, compute_throughput_history,
      # compute_thread_capacity, compute_worker_apps, compute_db_connections_by_app,
      # compute_dyno_info_by_app, compute_worker_dynos
