...
class SyncEmailsToSharePointJob < ApplicationJob
  ...
  def perform(credential_id)
...

    {
      ...
      attachments_uploaded: attachment_result[:uploaded],
  ...
  def sync_attachments_to_sharepoint
    ...
    # We'll look for emails from users in this org's tenant
    ...
    emails_with_attachments.each do |email|
      begin
        ...
        attachments.each do |attachment|
          # Only upload file attachments (skip inline/embedded)
          next unless attachment["@odata.type"] == "#microsoft.graph.fileAttachment"

          filename = attachment["name"]
          content_bytes = attachment["contentBytes"]

          # Skip if already uploaded (check by filename and email)
          # In a real implementation, you'd track uploaded attachments in a separate table

          # Upload to SharePoint
          # For now, we'll just log it - full implementation would use SharePoint API
          Rails.logger.info "[SyncToSharePoint] Would upload: #{filename} from #{email.subject}"
          uploaded += 1
        end
      rescue StandardError => e
