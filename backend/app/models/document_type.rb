class DocumentType < ApplicationRecord
  ...
  # SSoT: WarehouseFolder associations (Feb 2026 - THE ONE for folder config)
  ...
  def derived_scope
    case primary_warehouse_folder&.warehouse_type_code
  ...
  def target_folder
    primary_warehouse_folder&.full_folder_path || read_attribute(:target_folder)
  end
  ...
  # SSoT: Aliases are stored in the `aliases` JSONB column on each DocumentType record.
  # DEFAULT_ALIASES constant removed (Feb 2026) - migrated to DB via
  # 20260215200001_populate_document_type_aliases.rb
  ...
  # Find a document type by name OR any of its aliases (SSoT: aliases JSONB column)
  ...
  # Get all terms that map to this document type (name + aliases from DB)
  def all_terms
    db_aliases = aliases.is_a?(Array) ? aliases : []
    ([ name ] + db_aliases).uniq
  end
