...
class StorageLocation < ApplicationRecord
  ...
  # SSoT: StorageLocation can only add/remove SECONDARY links (is_primary: false)
  # Primary links are controlled from DocumentType side only
  def document_type_ids=(ids)
    ids = Array(ids).map(&:to_i).reject(&:zero?)
    existing_ids = storage_location_document_types.pluck(:document_type_id)

    # Remove old assignments - ONLY secondary ones!
    # Primary links cannot be removed from StorageLocation side
    ...
    (ids - existing_ids).each do |doc_type_id|
      storage_location_document_types.create(document_type_id: doc_type_id, is_primary: false)
    end
  end

  # Get document type IDs
  def document_type_ids
    storage_location_document_types.pluck(:document_type_id)
  end
  ...
  #
  # Examples:
  #   StorageLocation.folder_name_for("job", "plans")     # => "04 Plans" (from StorageLocation)
  #   StorageLocation.folder_name_for("job", "documents") # => "Documents"
  #   StorageLocation.folder_name_for("job", "missing", "Fallback") # => "Fallback"
  #
  ...
  # SSoT: Map StorageLocation to StorageConfiguration template warehouse type
  ...
  def warehouse_base_path
    ...
    Rails.logger.warn "[StorageLocation] Failed to get base path: #{e.message}"
    nil
  end
  ...
  def as_nested_json
    {
      ...
      document_types: document_types.map { |dt|
        join = storage_location_document_types.find_by(document_type_id: dt.id)
        {
          id: dt.id,
          name: dt.name,
  ...
  # SSoT: Get warehouse folder paths from StorageLocation
  ...
  #
  # Note: Legacy scope_folders used underscores (user_photos), but StorageLocation tab_key uses hyphens (user-photos).
  ...
  def self.seed_system_tabs!
    ...
    seed_corporate_storage_locations!
    ...
    Rails.logger.info "[StorageLocation] Seeded #{count} total tabs"
  end
  ...
  private_class_method def self.seed_task_tabs!
    ...
    Rails.logger.info "[StorageLocation] Seeded #{where(warehouse_type: 'task').count} task tabs"
  end
  ...
  private_class_method def self.seed_task_attachments_tabs!
    ...
    Rails.logger.info "[StorageLocation] Seeded #{where(warehouse_type: 'task_attachments').count} task_attachments tabs"
  end
  ...
  private_class_method def self.seed_task_responses_tabs!
    ...
    Rails.logger.info "[StorageLocation] Seeded #{where(warehouse_type: 'task_responses').count} task_responses tabs"
  end
  ...
  private_class_method def self.seed_email_tabs!
    ...
    Rails.logger.info "[StorageLocation] Seeded #{where(warehouse_type: 'email').count} email tabs"
  end
  ...
  # SSoT: Migrates all legacy scope_folders to StorageLocation entries
  # These tabs use tab_key as the storage lookup key (not warehouse_type)
  #
  # Legacy scope_folders keys mapped to StorageLocation:
  ...
  private_class_method def self.seed_legacy_storage_tabs!
    ...
    Rails.logger.info "[StorageLocation] Seeded #{where(warehouse_type: 'warehouse', tab_group: 'system').count} legacy storage tabs"
  end
  ...
  def inherit_warehouse_from_parent
    ...
    if parent&.warehouse_enabled
      ...
      Rails.logger.info "[StorageLocation] Auto-inherited warehouse_enabled from parent '#{parent.display_name}' for tab '#{display_name}'"
    end
  end
  ...
  def icon_uniqueness_for_root_tabs
    ...
    existing = StorageLocation.where(warehouse_type: warehouse_type, parent_id: nil, icon_name: icon_name)
                        .where.not(id: id)
