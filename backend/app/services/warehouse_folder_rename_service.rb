...
# WarehouseFolderRenameService - DB-only virtual folder path updates
#
...
#
# When WarehouseFolder.display_name changes, this service:
...
#
# Usage:
#   service = WarehouseFolderRenameService.new(
#     warehouse_folder: tab,
...
#
class WarehouseFolderRenameService
  def initialize(warehouse_folder:, old_display_name:, new_display_name:)
  ...
  def execute
    return error_result("Not a job-scope tab") unless @warehouse_folder.warehouse_type == 'job'
    return error_result("No storage folder configured") unless @warehouse_folder.warehouse_enabled

    # Compute old and new folder paths
    @old_path = compute_old_folder_path
    @new_path = @warehouse_folder.upload_folder_path

    Rails.logger.info "[WarehouseFolderRename] DB-only path update: '#{@old_path}' → '#{@new_path}'"
    ...
    Rails.logger.info "[WarehouseFolderRename] Complete: #{@stats.inspect}"
    { success: true, stats: @stats }
  rescue StandardError => e
    Rails.logger.error "[WarehouseFolderRename] Error: #{e.message}"
  ...
  # Logic mirrors WarehouseFolder#upload_folder_path but with old_display_name
  def compute_old_folder_path
    if @warehouse_folder.parent&.warehouse_enabled
      # Child tab: parent_path + "/" + old_display_name
      parent_path = @warehouse_folder.parent.upload_folder_path
  ...
  def update_warehouse_document_folders
    ...
    Rails.logger.info "[WarehouseFolderRename] Updated #{affected_count} WarehouseDocument folders"
  end
  ...
  def update_job_document_paths
    ...
    Rails.logger.info "[WarehouseFolderRename] Updated #{affected_count} JobDocument folder paths"
  end
  ...
  def cascade_to_child_tabs
    child_tabs = @warehouse_folder.children.where(warehouse_enabled: true)
    return if child_tabs.empty?

    Rails.logger.info "[WarehouseFolderRename] Cascading to #{child_tabs.count} child tabs"
