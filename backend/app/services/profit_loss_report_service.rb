...
# Generates a PDF Profit & Loss report from Xero API data.
...
class ProfitLossReportService
  ...
  POSITIVE_COLOR = "276749"   # Green for profits
  NEGATIVE_COLOR = "C53030"   # Red for losses
  ...
  def generate
    ...
    Rails.logger.error("ProfitLossReportService error: #{e.message}")
  ...
  def upload_to_sharepoint(content, filename)
    ...
    unless warehousing_folder
      ...
      Rails.logger.info("[ProfitLossReportService] Created SharePoint folder: Warehousing")
    end
    ...
    Rails.logger.info("[ProfitLossReportService] Uploaded to SharePoint: Warehousing/Financial Reports/#{company_name}/#{@report.financial_year}/#{filename}")
    upload_result
  rescue MicrosoftGraphClient::AuthenticationError => e
    Rails.logger.error("[ProfitLossReportService] SharePoint auth error: #{e.message}")
    nil
  rescue MicrosoftGraphClient::APIError => e
    Rails.logger.error("[ProfitLossReportService] SharePoint API error: #{e.message}")
    nil
  rescue StandardError => e
    Rails.logger.error("[ProfitLossReportService] SharePoint upload error: #{e.message}")
    nil
  end
