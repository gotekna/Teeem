...
class CaseFolderService
  ...
  def create_case_folder
    ...
    begin
      ...
      if result && result["id"]
        ...
        Rails.logger.info "[CaseFolderService] Created OneDrive folder: #{full_path}"
        result
      else
  ...
  def get_case_folder
    return nil unless @case.onedrive_folder_id.present?

    begin
      graph_client.get_item(@case.onedrive_folder_id)
    rescue MicrosoftGraphClient::APIError => e
  ...
  def ensure_folder_exists
    return get_case_folder if @case.onedrive_folder_id.present?
