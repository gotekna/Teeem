class AddPdfEligibilityToCorporateCompanyDocuments < ActiveRecord::Migration[8.0]
  def change
    ...
    add_column :corporate_documents, :is_pdf_eligible, :boolean, default: true, null: false
    add_column :corporate_documents, :orphaned_at, :datetime
    add_column :corporate_documents, :orphan_reason, :string

    # Index for fast eligibility queries
    add_index :corporate_documents, :is_pdf_eligible, where: "source = 'xero'"
    add_index :corporate_documents, :orphaned_at, where: "orphaned_at IS NOT NULL"
  end
end
