...
# Updates xero_sync_contacts_view to dynamically calculate match_confidence
# when it's NULL, using PostgreSQL's similarity() function from pg_trgm.
# This ensures all linked contacts show a confidence score based on name matching.
class AddDynamicMatchConfidenceToXeroSyncView < ActiveRecord::Migration[8.0]
  def up
    execute <<-SQL
      ...
      SELECT
        ...
        cel.needs_review,
        cel.match_type,
        -- Dynamic match_confidence: use stored value if present, otherwise calculate from name similarity
        COALESCE(
  ...
  def down
    # Restore previous version without dynamic match_confidence
    execute <<-SQL
      DROP VIEW IF EXISTS xero_sync_contacts_view;
      CREATE VIEW xero_sync_contacts_view AS
