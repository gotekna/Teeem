...
# Adds xero_link_count column to show how many Xero accounts
# are linked to the same TEEEM contact
class AddXeroLinkCountToXeroSyncContactsView < ActiveRecord::Migration[7.1]
  def up
    execute <<-SQL
  ...
  def down
    # Restore previous version without xero_link_count
    execute <<-SQL
      DROP VIEW IF EXISTS xero_sync_contacts_view;
      CREATE VIEW xero_sync_contacts_view AS
      SELECT
