...
class AddSyncKeyToClaimStageTemplates < ActiveRecord::Migration[7.1]
  def up
    add_column :claim_stage_templates, :sync_key, :string
    ...
    # Backfill existing records with parameterized name
    execute <<~SQL
  ...
  def down
    remove_index :claim_stage_templates, name: "idx_claim_stage_templates_on_tenant_sync_key"
    remove_column :claim_stage_templates, :sync_key
  end
end
