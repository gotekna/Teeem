...
# FRC (Feb 2026): Fix xero_sync_contacts_view after tenant_id→xero_org_id rename.
#
# Root cause: Migration 20260202224435 renamed contact_external_links.tenant_id
# to xero_org_id, but never recreated this view. The view still referenced
# cel.tenant_id which no longer exists.
#
# Also fixes the join condition on external_invoices: was using ei.tenant_id
# (TEEEM bigint FK) to match cel.tenant_id (Xero UUID string) - these are
# different things. Now correctly uses ei.xero_org_id = cel.xero_org_id.
class FixXeroSyncContactsViewColumnRename < ActiveRecord::Migration[8.0]
  def up
    ...
    execute <<-SQL
      ...
      SELECT
        ...
        cel.xero_org_id AS xero_tenant_id,
        ...
        -- Invoice/Bill counts (filtered by THIS Xero org)
        ...
        -- PDF sync stats (filtered by THIS Xero org)
        ...
        -- Blob health stats (filtered by THIS Xero org)
      ...
      -- Invoice/bill stats subquery (filtered by THIS Xero org)
      LEFT JOIN LATERAL (
        SELECT
        ...
        WHERE ei.contact_id = c.id
          AND ei.xero_org_id = cel.xero_org_id
      ) inv_stats ON true

      -- PDF stats subquery (filtered by THIS Xero org)
      LEFT JOIN LATERAL (
        SELECT
  ...
  def down
    # Revert would need the old column name which no longer exists
    # Just drop the view - it will be recreated by running migrations forward
    execute "DROP VIEW IF EXISTS xero_sync_contacts_view"
  end
end
