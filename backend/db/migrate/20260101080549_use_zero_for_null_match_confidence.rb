...
# Changes match_confidence from NULL to 0 for unlinked Xero contacts.
# This allows filtering/searching on match_confidence = 0 to find unlinked contacts.
class UseZeroForNullMatchConfidence < ActiveRecord::Migration[8.0]
  def up
    execute <<-SQL
      ...
      SELECT
        ...
        cel.needs_review,
        cel.match_type,
        -- Calculate match_confidence dynamically, default to 0 if can't calculate
        COALESCE(
          CASE
  ...
  def down
    # Restore previous version with NULL for unlinked contacts
    execute <<-SQL
      DROP VIEW IF EXISTS xero_sync_contacts_view;
      CREATE VIEW xero_sync_contacts_view AS
