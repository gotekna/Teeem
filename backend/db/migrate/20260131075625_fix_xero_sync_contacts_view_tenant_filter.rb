...
# Fix xero_sync_contacts_view to filter invoice/bill/PDF counts by tenant.
#
# BUG: If a TEEEM contact is linked to Xero contacts in multiple orgs,
# the view was counting invoices/bills/PDFs from ALL orgs instead of just
# the current tenant. This caused mismatched totals in the drilldown UI.
#
# FIX: Add tenant_id filter to invoice stats and PDF stats subqueries.
class FixXeroSyncContactsViewTenantFilter < ActiveRecord::Migration[8.0]
  def up
    execute <<-SQL
      ...
      SELECT
        ...
        -- Invoice/Bill counts (filtered by THIS tenant, not all orgs)
      ...
      -- FIX: Added ei.tenant_id = cel.tenant_id to only count docs from this Xero org
      LEFT JOIN LATERAL (
        SELECT
      ...
      -- FIX: Added ei.tenant_id = cel.tenant_id to only count PDFs from this Xero org
      LEFT JOIN LATERAL (
        SELECT
  ...
  def down
    # Revert to previous version without tenant filter
    execute <<-SQL
      CREATE OR REPLACE VIEW xero_sync_contacts_view AS
      SELECT
        cel.id AS id,
