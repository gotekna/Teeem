...
# Updates xero_sync_contacts_view to ALWAYS calculate match_confidence dynamically
# from the current TEEEM contact display_name vs Xero external_name.
# This ensures match_confidence updates when names are changed, rather than
# using stale stored values from when the link was first created.
class AlwaysCalculateMatchConfidenceDynamically < ActiveRecord::Migration[8.0]
  def up
    execute <<-SQL
      ...
      SELECT
        ...
        cel.needs_review,
        cel.match_type,
        -- ALWAYS calculate match_confidence dynamically from current names
        CASE
          WHEN c.display_name IS NOT NULL AND cel.external_name IS NOT NULL
          THEN similarity(LOWER(c.display_name), LOWER(cel.external_name))
          ELSE NULL
  ...
  def down
    # Restore version with COALESCE (uses stored value if present)
    execute <<-SQL
      DROP VIEW IF EXISTS xero_sync_contacts_view;
      CREATE VIEW xero_sync_contacts_view AS
