...
namespace :blob do
  namespace :cleanup do
    ...
    task :orphaned, [:mode, :age_days] => :environment do |_t, args|
      ...
      all_referenced_ids = (warehouse_blob_ids + corp_blob_ids + chat_blob_ids + bill_blob_ids).uniq
  ...
  namespace :audit do
    ...
    task :integrity, [:fix] => :environment do |_t, args|
      ...
      StorageBlob.find_each do |blob|
        ...
        actual_count = warehouse_count + corp_count + chat_count + bill_count
  ...
  task health: :environment do
    ...
    # Reference integrity quick check (SSoT: WarehouseDocument + legacy)
    mismatched = 0
