class UserOutlookCredential < ApplicationRecord
  ...
  def refresh!
    ...
    if response.status.success?
      ...
      Rails.logger.info "Outlook token refreshed successfully for user #{user_id}"
      true
    else
      Rails.logger.error "Failed to refresh Outlook token for user #{user_id}: #{response.status} - #{response.body}"
      false
    end
  rescue => e
    Rails.logger.error "Error refreshing Outlook token for user #{user_id}: #{e.message}"
    false
  end
