...
This document provides the technical implementation details for detecting bug patterns in the TEEEM codebase. Each pattern has:
...
import TEEEMTableView from './TEEEMTableView'  // ❌ No match

<TablePage data={data} />                     // ✅ Match
<TEEEMTableView data={data} />               // ❌ No match
```
...
function validateDeprecatedTable(fileContent, match) {
  ...
  const hasTodoComment = /TODO.*TEEEMTableView|migrate.*table/i.test(fileContent)
...
function provideMigrationGuidance(match, fileContent) {
  return {
    ...
    steps: [
      ...
      '2. Replace TablePage import with TEEEMTableView',
...
test('ignores TEEEMTableView import', () => {
  const code = `import TEEEMTableView from './TEEEMTableView'`
  expect(detectPattern(code)).toBe(false)
})
```
...
- **Pattern Library:** `/TEEEM_DOCS/PATTERN_LIBRARY.md`
